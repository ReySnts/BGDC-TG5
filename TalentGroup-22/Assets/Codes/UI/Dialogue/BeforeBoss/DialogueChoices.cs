...
public class DialogueChoices : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        _StoryScript.BindExternalFunction
        (
            "Name", 
            (string charName) => ChangeName(charName)
        );
        _StoryScript.BindExternalFunction
        (
            "Icon", 
            (string charName) => ChangeCharacterIcon(charName)
        );
        DisplayNextLine();
        #endregion
    ...
    public void DisplayNextLine()
    {
        ...
        if (_StoryScript.canContinue) // Checking if there is content to go through
        {
            ...
            StartCoroutine
            (
                PlayDialogue(dialogueBox.text = text)
            );
        }
        else if 
        (
            _StoryScript.currentChoices.Count > 0 
            && 
            choiceHolder.GetComponentsInChildren<Button>().Length <= 0
        ) 
        {
        ...
        else SceneManager.LoadScene("GoodEnding");
    }
    IEnumerator PlayDialogue(string line)
    {
        ...
        foreach 
        (
            char letter in line.ToCharArray()
        )
        {
    ...
    Button CreateChoiceButton(string text)
    {
        ...
        choiceButton.transform.SetParent
        (
            choiceHolder.transform, 
            false
        );
    ...
    void OnClickChoiceButton(Choice choice)
    {
        if (canContinueToNextLine)
        {
            ...
            if (choiceHolder != null) 
                foreach 
                (
                    var button in choiceHolder.GetComponentsInChildren<Button>()
                )
            ...
            if (choice.index == 0) //YES CHOICE
            {
                ...
                StartCoroutine
                (
                    BadEndDialog(textSegment[0].Dialogue)
                );
            }
        }
    }
    ...
    {
        yield return new WaitForSeconds(10f / textSpeed);
